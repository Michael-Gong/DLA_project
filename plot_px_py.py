...
#  nsteps      = 79580 #sum(1 for line in open(from_path+'x_0000.txt'))/part_number
  ...
  for n in range(part_number):
      ...
      plt.scatter(px[n,:], py[n,:], c=gg[n,:], norm=colors.Normalize(vmin=0,vmax=1500), s=10, cmap='rainbow', edgecolors='None', alpha=1,zorder=3)
...
#  cbar.set_label('$\gamma$',fontdict=font)
      ...
      plt.scatter(xx[n,:], yy[n,:], c=gg[n,:], norm=colors.Normalize(vmin=0,vmax=1500), s=10, cmap='rainbow', edgecolors='None', alpha=1,zorder=3)
#  cbar=plt.colorbar( ticks=np.linspace(0, 500, 3) ,pad=0.005)
#  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=font_size)
#  cbar.set_label('$\gamma$',fontdict=font)
...
#  cbar.set_label('$\gamma$',fontdict=font)
      ...
      fig.savefig(to_path+'trace_particle'+str(n).zfill(2)+'.png',format='png',dpi=160)
      plt.close("all")
