...
for n_snap in range(0, 50000, 500):
	print('This is '+str(n_snap).zfill(5)+' of '+str(20000))
        ...
        plt.scatter(t[n_min:n_max]/2/np.pi,w[n_min:n_max], c=lgR, s=5, cmap='magma_r', edgecolors='None')
        plt.scatter(t[n_min+n_snap]/2/np.pi,w[n_min+n_snap], color='lime', marker='o', s=200, edgecolors='k')
        ...
        plt.scatter(t[n_min:n_max]/2/np.pi,d_work[n_min:n_max], c=lgR, s=5, cmap='magma_r', edgecolors='None')
        plt.scatter(t[n_min+n_snap]/2/np.pi,d_work[n_min+n_snap], color='lime', marker='o', s=200, edgecolors='k')
        ...
        plt.scatter(t[n_min:n_max]/2/np.pi,gamma[n_min:n_max], c=lgR, s=5, cmap='magma_r', edgecolors='None')
        plt.scatter(t[n_min+n_snap]/2/np.pi,gamma[n_min+n_snap], color='lime', marker='o', s=200, edgecolors='k')
        #plt.plot((t[index,:])/2/np.pi,np.sqrt(px[index,:]**2+py[index,:]**2+1),'--k',linewidth=2.5,label='No RR')
        #plt.legend(loc='upper right')
        #cbar=plt.colorbar(ticks=np.linspace(np.min(gamma), np.max(gamma), 5))
        ...
        plt.contour(p_px, p_py, p_R, np.array([5.0]),linestyles='dashed',linewidth=0.5, alpha=0.35)
        #plt.scatter(px2[n_min2:n_max2],py2[n_min2:n_max2], c=lgR2, s=3, cmap='plasma_r', edgecolors='None')
        plt.scatter(px[n_min:n_max],py[n_min:n_max], c=lgR, s=3, cmap='magma_r', edgecolors='None')
        plt.scatter(px[n_min+n_snap],py[n_min+n_snap], color='lime', marker='o', s=200, edgecolors='k')
        #cbar=plt.colorbar(ticks=np.linspace(np.min(lgR), np.max(lgR), 5))
        #cbar.set_label(r'$R$', fontdict=font)#plt.xlim(47,53)
        ...
        plt.ylim(-245.0,245.0)
        plt.xlim(-99.0,2499.0)
        plt.legend(loc='best')
        ...
        fig.savefig('./series_LD/series'+str(n_snap).zfill(6)+'.png',format='png',dpi=160)
        plt.close("all")
